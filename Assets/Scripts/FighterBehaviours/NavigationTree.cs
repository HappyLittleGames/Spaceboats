        ...
        private BHStatus GetDestinationToTarget()
        {
            if (m_blackboard.target != null)
            {
                if (m_blackboard.target.GetComponent<Rigidbody>() != null)
                {
                    ...
                    m_desiredDirection = (m_blackboard.target.transform.position + m_blackboard.target.GetComponent<Rigidbody>().velocity * m_blackboard.tickInterval - m_propulsion.rigidbody.velocity * m_blackboard.tickInterval) - m_propulsion.rigidbody.position + m_blackboard.fighter.spaceManager.GetGravity(m_propulsion.rigidbody.position);
